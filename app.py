...
<body>
    ...
    <div id="auth-wall" class="relative z-10 flex items-center justify-center min-h-screen">
        ...
        <div class="lux-card p-10 rounded-[2rem] shadow-2xl w-96 text-center">
            ...
            <h2 class="text-3xl font-bold mb-8 tracking-tight">BodhiHub</h2>
    ...
    <div id="main-content" class="hidden relative z-10 p-10">
        ...
        <div class="absolute top-10 right-10 flex items-center gap-4">
            ...
            <button onclick="toggleSearch()" class="p-3 bg-white/5 hover:bg-white/10 rounded-full transition">
        ...
        <h1 class="text-5xl font-light text-center mb-12 tracking-[0.2em] uppercase text-white/90">

            Bodhi<span class="font-bold text-transparent bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text">Hub</span>
    ...
    <script>
        ...
        const categoryConfig = { 
            ...
            'Student': { 'color': 'blue', 'icon': 'book-open' }, 

            'Engineer': { 'color': 'emerald', 'icon': 'code' }, 

            'Editor': { 'color': 'violet', 'icon': 'film' },

            'Teacher': { 'color': 'amber', 'icon': 'graduation-cap' }, 

            'Doctor': { 'color': 'rose', 'icon': 'stethoscope' }, 

            'General': { 'color': 'slate', 'icon': 'bot' } 
        ...
        function searchTools() {
            ...
            render(allTools.filter(t => t.name.toLowerCase().includes(val)));
        ...
        function filterTools(cat) {
            ...
            document.querySelectorAll('.filter-btn').forEach(btn => {
                ...
                btn.className = btn.innerText === cat ? 'filter-btn bg-blue-600 px-6 py-2.5 rounded-full font-medium transition' : 'filter-btn bg-white/5 hover:bg-white/10 px-6 py-2.5 rounded-full font-medium transition';
...
def signup():
    ...
    if User.query.filter_by(email=data['email']).first(): return jsonify({"status": "error", "message": "User already exists"})
